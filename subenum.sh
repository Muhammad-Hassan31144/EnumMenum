...
CheckDependencies(){
	# List of required tools based on setup.sh and script usage
	tools=(
		"findomain"
		"subfinder"
		"amass"
	...
	for tool in "${tools[@]}"; do
		if ! command -v "$tool" &> /dev/null; then
			echo -e "${red}[-] Missing required tool: $tool${end}"
			missing_tools=true
		fi
	done

	if [ "$missing_tools" = true ]; then
		echo -e "\n${Underlined}${red}[!] Tools missing!${end}"
		echo -e "It seems like you haven't installed the required tools or they are not in your \$PATH."
		echo -e "This script relies on external tools to function correctly."
		echo -e "\n${bold}Please install the missing tools.${end}"
		echo -e "You can follow the installation guide at: ${blue}https://github.com/bing0o/SubEnum${end}"
		echo -e "Or run the bundled setup script: ${green}./setup.sh${end}"
		exit 1
	fi
}
